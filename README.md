...
The current 40 tool identities and canonical tool URLs are real, but the
Telegram channel identities, source-post URLs, presentation dates, and
collection dates are explicit interface-development placeholders. The current
retrieval evaluation cases are also temporary behavior checks, not
